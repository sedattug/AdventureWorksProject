...
var dNow=new Date();rd.push('&xjxr=');rd.push(dNow.getTime());delete dNow;var csrf = xx.config.ci_default_token;
 
rd.push('&ci_default_token=');
 
rd.push(csrf);
 
delete csrf;if(oRequest.parameters){var i=0;var iLen=oRequest.parameters.length;while(i < iLen){var oVal=oRequest.parameters[i];if('object'==typeof oVal&&null!=oVal){try{oVal=JSON.stringify(oVal);}catch(e){oVal='';}
