...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        if ($this->config->item('rest_ip_blacklist_enabled') !== null) {
    ...
    public function response($data = null, $http_code = null, $continue = false) {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            if ($this->config->item('compress_output') !== null && $this->config->item('compress_output') === true && $this->_zlib_oc == false) {
        ...
        if (!$this->_zlib_oc && $this->config->item('compress_output') !== null) {
    ...
    protected function _detect_output_format() {
        ...
        if ($this->config->item('rest_ignore_http_accept') === null and $this->input->server('HTTP_ACCEPT')) {
            // Check all formats against the HTTP_ACCEPT header
    ...
    protected function _detect_method() {
        ...
        if ($this->config->item('enable_emulate_request') !== null) {
    ...
    protected function _log_request($authorized = false) {
        ...
        $this->_insert_id = $this->rest->db->last_insert_id('logs', 'log_id');
        return $status;
    }
    ...
    protected function _check_login($username = '', $password = false) {
        ...
        if ($this->config->item('auth_source') !== null && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
    ...
    protected function _force_login($nonce = '') {
        if ((string) strtolower($this->config->item('rest_auth')) === 'basic') {
            header('WWW-Authenticate: Basic realm="' . $this->config->item('rest_realm') . '"');
        } elseif ((string) strtolower($this->config->item('rest_auth')) === 'digest') {
