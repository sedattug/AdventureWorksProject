...
final class xajaxArgumentManager {
    ...
    private function __convertValue($value) {
        ...
        switch ($cType) {
            case 'S': $value = false === $sValue ? '' : $sValue;
                break;
            case 'B': $value = $this->__convertStringToBool($sValue);
                break;
            case 'N': $value = $sValue === floor($sValue) ? (int) $sValue : (float) $sValue;
                break;
            case '*': $value = null;
                break;
        }
    ...
    private function __construct() {
        ...
        if ($this->ci->input->post('xjxargs') !== null) {
            $this->nMethod = XAJAX_METHOD_POST;
            $this->aArgs = $this->ci->input->post('xjxargs', true);
        } else if ($this->ci->input->get('xjxargs', true) !== null) {
            $this->nMethod = XAJAX_METHOD_GET;
            $this->aArgs = $this->ci->input->get('xjxargs');
        }
