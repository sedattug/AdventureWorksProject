...
//EndSkipAIO
    ...
    public function processRequest() {
        $HTTP_CHALLENGE_RESPONSE = librid_server_variables('HTTP_CHALLENGE_RESPONSE');
        if (isset($HTTP_CHALLENGE_RESPONSE))
            $this->challengeResponse = $HTTP_CHALLENGE_RESPONSE;
...
//EndSkipDebug
            ...
            if ($this->bErrorHandler) {
                ...
                if (!empty($sErrorMessage)) {
                    if (0 < strlen($this->sLogFile)) {
                        ...
                        if (NULL !== $fH) {
                            fwrite(
                                    $fH, $this->objLanguageManager->getText('LOGHDR:01')
    ...
    private function _detectURI() {
        ...
        $_LIBRID_SERVER = librid_server_variables();
        // Try to get the request URL
        if (!empty($_LIBRID_SERVER['REQUEST_URI'])) {

            $_LIBRID_SERVER['REQUEST_URI'] = str_replace(
                    array('"', "'", '<', '>'), array('%22', '%27', '%3C', '%3E'), $_LIBRID_SERVER['REQUEST_URI']
            );

            $aURL = parse_url($_LIBRID_SERVER['REQUEST_URI']);
        }

        // Fill in the empty values
        if (empty($aURL['scheme'])) {
            if (!empty($_LIBRID_SERVER['HTTP_SCHEME'])) {
                $aURL['scheme'] = $_LIBRID_SERVER['HTTP_SCHEME'];
            } else {
                $aURL['scheme'] = (FORCE_SSL === true || (!empty($_LIBRID_SERVER['HTTPS']) && strtolower($_LIBRID_SERVER['HTTPS']) !== 'off')) ? 'https' : 'http';
            }
        }

        if (empty($aURL['host'])) {
            if (!empty($_LIBRID_SERVER['HTTP_X_FORWARDED_HOST'])) {
                if (strpos($_LIBRID_SERVER['HTTP_X_FORWARDED_HOST'], ':') > 0) {
                    list($aURL['host'], $aURL['port']) = explode(':', $_LIBRID_SERVER['HTTP_X_FORWARDED_HOST']);
                } else {
                    $aURL['host'] = $_LIBRID_SERVER['HTTP_X_FORWARDED_HOST'];
                }
            } else if (!empty($_LIBRID_SERVER['HTTP_HOST'])) {
                if (strpos($_LIBRID_SERVER['HTTP_HOST'], ':') > 0) {
                    list($aURL['host'], $aURL['port']) = explode(':', $_LIBRID_SERVER['HTTP_HOST']);
                } else {
                    $aURL['host'] = $_LIBRID_SERVER['HTTP_HOST'];
                }
            } else if (!empty($_LIBRID_SERVER['SERVER_NAME'])) {
                $aURL['host'] = $_LIBRID_SERVER['SERVER_NAME'];
            } else {
        ...
        if (empty($aURL['port']) && !empty($_LIBRID_SERVER['SERVER_PORT'])) {
            $aURL['port'] = $_LIBRID_SERVER['SERVER_PORT'];
        }
        ...
        if (empty($aURL['path'])) {
            $sPath = array();
            if (!empty($_LIBRID_SERVER['PATH_INFO'])) {
                $sPath = parse_url($_LIBRID_SERVER['PATH_INFO']);
            } else {
                $sPath = parse_url($_LIBRID_SERVER['PHP_SELF']);
            }
            if (isset($sPath['path']))
        ...
        if (empty($aURL['query']) && !empty($_LIBRID_SERVER['QUERY_STRING'])) {
            $aURL['query'] = $_LIBRID_SERVER['QUERY_STRING'];
        }
        ...
        if (!empty($aURL['port'] && FORCE_SSL === false) && (($aURL['scheme'] == 'http' && (integer) $aURL['port'] !== 80) || ($aURL['scheme'] === 'https' && (integer) $aURL['port'] !== 443))) {
            $sURL .= ':' . $aURL['port'];
        }
